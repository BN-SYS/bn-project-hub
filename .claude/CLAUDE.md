...
| View | `echo`, 단순 조건 출력만 — DB·비즈니스 로직 금지 |
| Helper | 동일 로직 2회 이상 반복 시 반드시 분리 |

### View 파일 작업 경계 (PM ↔ 개발자 충돌 방지)
PM 산출물(HTML 구조·클래스명)이 PHP View로 전환된 이후에도 역할 경계를 유지한다.
**목적**: 서로의 작업 영역을 보호해 양쪽 모두 안전하게 작업하기 위함.
- PM이 PHP 태그를 실수로 건드리면 서버 오류 → HTML 영역만 담당
- 개발자가 HTML 구조·클래스를 바꾸면 퍼블 깨짐 → PHP 출력 영역만 담당

| 작업 주체 | 담당 영역 |
|----------|----------|
| **PM** | HTML 구조, CSS 클래스명, 외부 CSS/JS 파일 수정 |
| **개발자** | `<?php echo $var ?>` 변수 출력, 단순 조건(`<?php if ?>`) 추가 |

**개발자가 자주 맞닥뜨리는 케이스 처리법**
- 동적으로 스타일 변경 필요 → 인라인 스타일 금지, CSS 클래스를 PHP로 토글
  ```php
  <div class="card <?php echo $isActive ? 'is-active' : '' ?>">
  ```
- HTML 구조 변경이 필요한 경우 → PM과 협의 후 반영 (구조 변경은 퍼블 영향 있음)

### assets 관리 규칙
- CSS/JS는 반드시 `public/assets/` 하위
- 인라인 스타일·스크립트 금지 (아래 CSS/JS 분리 원칙 참조)
...
## CSS/JS 분리 원칙 (신규 프로젝트 적용)
배포 후 개발자와 동일 파일 충돌을 최소화하기 위해 HTML·CSS·JS를 완전 분리한다.

| 구분 | HTML 허용 여부 |
|------|--------------|
| `<script src="...">` 외부 파일 로드 | ✅ 허용 |
| `<link rel="stylesheet" href="...">` | ✅ 허용 |
| `<style> ... </style>` 블록 | ❌ 금지 — 외부 CSS 파일로 분리 |
| `<script> // 로직 </script>` 인라인 블록 | ❌ 금지 — 외부 JS 파일로 분리 |
| `onclick="..."` 인라인 이벤트 핸들러 | ❌ 금지 — JS에서 addEventListener 사용 |
| `style="..."` 인라인 스타일 | ❌ 금지 — CSS 클래스로 대체 |

### 고객 피드백 반영 시 규칙 (예외 없음)
피드백으로 색상·간격·폰트 등을 수정할 때도 인라인 스타일 사용 금지.
빠른 수정이 필요한 경우 아래 방법을 사용한다:
- **색상·브랜드값** → `common.css` 상단 CSS 변수(`--color-primary` 등) 수정
- **특정 요소 수정** → 해당 페이지 CSS 파일에 클래스 추가 또는 수정
- **JS 동작 수정** → 해당 JS 파일에서만 수정

> 기존 프로젝트(foresto 등)는 소급 적용하지 않는다.

---

## 키 파일 관리 규칙 (전 프로젝트 공통)

비밀번호·API 키 등 민감 정보는 반드시 아래 규칙을 따른다.

### 절대 커밋 금지 파일 (gitignore 적용)
| 파일 | 용도 |
|------|------|
| `config/config.php` | PHP 프로젝트 DB·사이트 설정 |
| `config/admin_profile.json` | 관리자 계정·비밀번호 해시 |
| `config/*.local.php` / `config/*.local.json` | 기타 로컬 전용 설정 |
| `.env` / `.env.local` | JS/HTML 프로젝트 API 키 |
| `_shared/_licensed/` | 유료 라이선스 에셋 (Velzon 등 신규 추가분) |

### 반드시 커밋할 파일 (구조 문서화)
| 파일 | 용도 |
|------|------|
| `config/config.example.php` | 필요한 상수 목록 + 채우는 방법 안내 |
| `config/admin_profile.example.json` | 관리자 프로필 구조 안내 |
| `.env.example` | 필요한 환경변수 목록 안내 |

### 프로젝트 초기화 시 pm-assistant 작업
1. `_shared/templates/config.example.php` → `dev/config/config.example.php` 복사
2. `_shared/templates/.env.example` → 프로젝트 루트 `.env.example` 복사
3. 신규 `dev/.gitignore`에 `config/config.php`, `config/admin_profile.json` 반드시 포함

### 코드에서 키 참조 방법
- **PHP**: `config/config.php`에서 `define()` 상수로 정의 → 코드에서 `DB_PASS` 등 상수 직접 사용
- **JS/HTML**: `.env` 파일에서 변수 정의 → JS에서 `import.meta.env.VITE_KEY` 또는 빌드 전 주입

---

## 폼 UX 기본 규칙 (전 프로젝트 공통)

기획·스토리보드·프로토타입 HTML 제작 시 아래를 기본으로 적용한다. 클라이언트 요청 시에만 예외 허용.

| 항목 | 기본 규칙 |
|------|----------|
| 폼 검증 메시지 | 인라인 메시지 (입력 필드 바로 아래 표시) — 팝업·모달 사용 안 함 |
| 폼 제출 버튼 | 필수 항목 입력 및 체크 완료 시에만 활성화 (기본 `disabled`) |
| 경고창 | `alert()` 기본 사용 |
| 토스트 메시지 | 원칙적으로 사용 안 함 — 요청 시에만 반영 |

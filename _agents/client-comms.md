...
## 실행 모드
이 에이전트는 **background 실행**을 기본으로 한다.
pm-assistant가 각 phase 완료 직후 즉시 스폰하며, 메인 체인을 블로킹하지 않는다.
pm-assistant로부터 `[N_phase 완료]` 인라인 정보가 함께 전달된다.
...
PM이 "공감형으로 바꿔줘" 또는 "적극형으로" 요청 시에만 추가 생성.
톤 설명:
- 정중형: 격식체, 신중
- 공감형: 감정 수용, 함께 해결
- 적극형: 직접적, 사실 중심, 즉각 조치
